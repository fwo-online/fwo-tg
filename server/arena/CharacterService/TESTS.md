...
Тесты используют правильные формулы расчета опыта с учетом `config.lvlRatio = 3`:

### Формула расчета уровня
```typescript
const calculateLvl = (exp: number) => {
  const k = 1000 * config.lvlRatio; // k = 3000
  return Math.max(1, Math.floor(Math.log2(exp / k) + 2));
};
```

### Формула расчета суммарного опыта для уровня
```typescript
const expToLevel = (lvl: number) => {
  const k = 1000 * config.lvlRatio; // k = 3000
  let sum = 0;
  for (let i = 2; i <= lvl; i++) {
    sum += k * 2 ** (i - 2);
...
### Таблица значений опыта

| Уровень | Суммарный опыт | Дополнительно для следующего уровня |
|---------|----------------|-------------------------------------|
| 1       | 0              | 3000                                |
| 2       | 3000           | 6000                                |
| 3       | 9000           | 12000                               |
...
**Формула дополнительного опыта:** `2^(lvl-1) * 1000 * lvlRatio`
