/**
 * Первичный загрузчик магий
 */
export { default as acidSpittle } from './acidSpittle';
export { default as anathema } from './anathema';
export { default as bleeding } from './bleeding';
export { blessing } from './blessing';
export { blight } from './blight';
export { default as bodySpirit } from './bodySpirit';
export { default as chainLightning } from './chainLightning';
export { curse } from './curse';
export { default as dispel } from './dispel';
export { dustShield } from './dustShield';
export { eclipse } from './eclipse';
export { default as entangle } from './entangle';
export { default as exorcism } from './exorcism';
export { default as fireBall } from './fireBall';
export { default as fireRain } from './fireRain';
export { frostTouch } from './frostTouch';
export { glitch } from './glitch';
export { lightHeal } from './lightHeal';
export { lightShield } from './lightShield';
export { madness } from './madness';
export { default as magicArmor } from './magicArmor';
export { default as magicArrow } from './magicArrow';
export { default as magicDefense } from './magicDefense';
export { magicWall } from './magicWall';
export { mediumAura } from './mediumAura';
export { default as mediumHeal } from './mediumHeal';
export { paralysis } from './paralysis';
export { default as physicalSadness } from './physicalSadness';
export { default as poisonBreath } from './poisonBreath';
export { default as rockfall } from './rockfall';
export { default as secondLife } from './secondLife';
export { silence } from './silence';
export { sleep } from './sleep';
export { smallAura } from './smallAura';
export { default as stoneSkin } from './stoneSkin';
export { strongAura } from './strongAura';
export { default as strongHeal } from './strongHeal';
export { default as vampirism } from './vampirism';
