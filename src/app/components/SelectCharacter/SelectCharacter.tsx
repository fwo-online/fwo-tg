import type { CreateCharacterDto, CharacterClass } from '@/schemas/character';
import {
...
import { characterClassImageMap, characterClassList, characterClassNameMap } from '@/app/constants/character';
import styles from './SelectCharacter.module.css';

const Slide = ({ value }: { value: CharacterClass }) => {
  return (
    <div className={styles.slide}>
      <img src={characterClassImageMap[value]}/>
      ...
      <Section header={<Section.Header large>Создайте вашего персонажа</Section.Header>}>
        <CompactPagination>
          {characterClassList.map((value, index) => (
            <CompactPaginationItem key={value} selected={selected === index} />
          ))}
        </CompactPagination>
