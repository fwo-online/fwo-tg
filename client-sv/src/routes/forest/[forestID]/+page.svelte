...
{:else}
  <Card header="Лес">
    <div class="mb-4 p-2 border-2 border-dashed">
      <Player name={name} {characterClass} />
    ...
    {#if lastResult}
      <div
        class={["mb-4 p-2 border-2", { 'border-green-500': lastResult.success, 'border-red-500': !lastResult.success }]}
      >
    ...
    {#if isEvent && status.currentEvent}
      <div class="text-center">
        ...
        <p class="text-sm mb-4">{EVENT_DESCRIPTIONS[status.currentEvent.type]}</p>
        <div class="text-xs mb-4">
          Осталось времени: {Math.max(0, Math.ceil((new Date(status.currentEvent.expiresAt).getTime() - Date.now()) / 1000))} сек
        </div>
