...
Система использует экспоненциальную прогрессию с коэффициентом `config.lvlRatio = 3`:
...
```typescript
const k = 1000 * 3 = 3000
lvl = Math.max(1, Math.floor(Math.log2(exp / k) + 2))
```

**Опыт до следующего уровня:**
```typescript
nextLevelExp = 2^(lvl-1) * 3000
```

### Таблица прогрессии

| Уровень | Суммарный опыт | Для следующего уровня |
|---------|----------------|-----------------------|
| 1       | 0              | 3,000                 |
| 2       | 3,000          | 6,000                 |
| 3       | 9,000          | 12,000                |
