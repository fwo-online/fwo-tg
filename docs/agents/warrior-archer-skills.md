...
- ⚡ **Прицельный выстрел** (`aimedShot`): тщательная наводка. Меткость +30%, крит `x1.5..x2.0` по одиночной цели.
- ⚡ **Метка охотника** (`huntersMark`): накладывает метку на 2 раунда. Весь входящий урон стрелами по цели увеличивается на +30%.
- ⚡ **Бронебойный выстрел** (`piercingShot`): выстрел тяжелой стрелой, игнорирующей 50–75% физической брони цели.
- ⚡ **Смертоносный выстрел** (`fatalShot`): добивающий выстрел. Наносит удвоенный урон по цели с HP < 30%.
...
- ⚡ **Рикошет** (`ricochet`): стрела поражает цель, после чего рикошетит во 2-го случайного врага (наносит 70% урона от первого удара).
- ⚡ **Залп стрел** (`doubleShot`): выстрел сразу по 2 случайным вражеским целям в раунде.
- ⚡ **Град стрел** (`arrowRain`): AoE-удар тучей стрел по всей вражеской команде.
- ⚡ **Зажигательная стрела** (`fireArrow`): выстрел с алхимическим зарядом: наносит физ. урон и поджигает цель (огонь DoT на 2 раунда).
...
- ⚡ **Увёртка** (`dodge`): уклонение от атак колющим, режущим, рубящим, метательным оружием (уже в коде).
- ⚡ **Полевая перевязка** (`fieldMedic` / `cure`): быстрое восстановление здоровья союзнику или себе прямо в бою.
- ⚡ **Подножка** (`step`): сбивание с ног: наносит урон и накладывает дебафф -30% к атаке и защите цели на 1 раунд.
- ⚡ **Дымовая завеса** (`smokeScreen`): укрытие в дыму: дает всей команде +30% к увороту на 1 раунд.
...
- ⚡ **Блок щитом** (`shieldBlock`): емкость блока щита поглощает удары, временно повышает магзащиту (уже в коде).
- ⚡ **Броня / Укрепление** (`armor`): селф-бафф: +50% к физической защите (`phys.defence`) на текущий раунд.
- ⚡ **Спасение** (`help`): таунт: принудительно перенаправляет все атаки с выбранного союзника на Воина.
- ⚡ **Удар щитом** (`shieldBash`): сокрушительный удар щитом: дробящий урон и шанс 50–75% оглушить цель (`stun`) на 1 ход.
- 🛡️ **Несокрушимость** (`unyielding`): *пассивный талант*. При падении HP ниже 30% весь входящий урон снижается на 40%.

#### 2. 🪓 Ярость / Варвар (`berserker`) — Разрушительный урон, сплеш, жертва защитой
- ⚡ **Берсерк** (`berserk`): резкое повышение максимального урона ценой снижения защиты (уже в коде).
- ⚡ **Размашистый удар** (`sweepingBlow`): активный клив: задевает стоящих рядом врагов на 50–75% урона от основной цели.
- ⚡ **Кровавая ярость** (`bloodRage`): воин жертвует 10% своего текущего HP ради баффа +40% к атаке и пробитию брони на 2 раунда.
- ⚡ **Казнь** (`execute`): сокрушительный добивающий удар: удваивает урон, если у цели осталось менее 35% HP.
...
- ⚡ **Парирование** (`parry`): парирование атак холодного оружия ближнего боя (уже в коде).
- ⚡ **Обезоруживание** (`disarm`): выбивает оружие цели при сравнении ловкости, запрещая атаку в раунде (уже в коде).
- ⚡ **Рассечение** (`lacerate`): точный порез, вызывающий сильное кровотечение `bleeding` на 3 раунда.
- ⚡ **Точный выпад** (`vitalStrike`): удар в уязвимое место в обход 50% защиты цели; если цель оглушена или обезоружена — наносит критический урон x1.75.
...
Ряд способностей принадлежит сразу двум специализациям и доступен при выборе любой из них:
- **`disarm` (Обезоруживание)**: `['duelist', 'scout']` (доступно Дуэлянту и Следопыту).
- **`help` (Спасение)**: `['guardian', 'scout']` (доступно Танку и Следопыту).
- **`sweepingBlow` (Размашистый удар)**: `['berserker', 'duelist']`.
- **`huntersMark` (Метка охотника)**: `['marksman', 'scout']`.
- **`ricochet` (Рикошет)**: `['barrage', 'marksman']`.
- **`shieldBlock` (Блок щитом)**: `['guardian', 'scout']` (при экипированном щите).
