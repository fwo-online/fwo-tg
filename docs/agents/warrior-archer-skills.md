...
- **Механика и архитектура**:
  - Архитектурно разделен на класс скилла `CripplingShot extends Skill` и класс дебаффа цели `CripplingShotDebuff extends BaseAction` (`isAffect = true`).
  - **`CripplingShot`**:
    ...
    - `onDamageDealt`: при попадании физической атакой оружием дальнего боя накладывает дебафф через `cripplingShotDebuff.apply(target, initiator, debuffPercent)`.
  - **`CripplingShotDebuff`**:
    - Сразу снижает ловкость цели (`attributes.dex`) на 25–45% в текущем раунде.
    - Накладывает долгий эффект `addLongEffect` на 2 раунда:
      - `onCast`: в начале каждого раунда повторно пересчитывает и накладывает срез ловкости на цель (так как между раундами статы сбрасываются в `StatsService.refresh()`).
      - `onBeforeAction`: если цель пытается применить «Увёртку» (`dodge`), формирует контекст с инициатором выстрела и целью и выбрасывает `new CastError(this.getSuccessResult(this.context))`. Благодаря этому уворот блокируется, а в боевой лог записывается причина: `_🎯 Выстрел в колено_ *Игрок 1*: 📖0`.
...
| **Выстрел в колено** | `cripplingShot`<br>(`cs`) | ⚡ Активное<br>*(Селф-бафф)* | 2 | 12 / 14 / 16 | 80 / 85 / 90 | Срез ловкости: `-25%` / `-35%` / `-45%` | Сбивает увёртку цели, срезает ловкость (`dex`) на 2 раунда (с переприменением в `onCast`) и блокирует «Увёртку» цели (`CastError` с причиной `getSuccessResult`). Логика дебаффа вынесена в `CripplingShotDebuff`. |
...
### 7.4. Паттерн «Селф-скилл + Класс дебаффа» (`CripplingShot` & `CripplingShotDebuff`)
Для скиллов, которые накладывают комплексные отрицательные эффекты на цель при попадании:
- Класс самого умения (`CripplingShot extends Skill`) отвечает за каст, списание энергии, бафф на стрелка и проверку попадания.
- Класс дебаффа (`CripplingShotDebuff extends BaseAction`) помечается `isAffect = true`, хранит логику модификации характеристик цели и хуки контроля:
  - `apply(target, initiator, debuffPercent)` — первичное наложение и добавление `addLongEffect`.
  - `onCast(target, affect)` — повторное наложение снижения статов в начале каждого раунда в стадии скилла.
  - `onBeforeAction(actionCtx, actionToCast, affect)` — проверка и срыв запрещённых действий с передачей `throw new CastError(this.getSuccessResult(this.context))`.
