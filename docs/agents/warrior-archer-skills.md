...
| **Прицельный выстрел** (`aimedShot`) | `as` | Лучник | ✅ **Реализовано** | `server/arena/skills/aimedShot.ts` | `profList: { l: 2 }`. Селф-бафф (`OrderType.Self`): усиливает урон (`x1.25..x1.5`), снимает и игнорирует увёртку цели (`dodge`), гарантируя попадание. Расход: 10–14 EN. |
...
| **Залп стрел** (`doubleShot`) | `ds` | Лучник | ✅ **Реализовано** | `server/arena/skills/doubleShot.ts` | `profList: { l: 2 }`. Селф-бафф (`OrderType.Self`): при выстреле лучник выпускает дополнительную стрелу во вторую цель (или ту же) на 60–90% урона. Расход: 12–16 EN. |
...
Каждая ветка Лучника содержит ровно **3 активных умения** и **2 пассивных таланта**:

#### 1. 🎯 Снайпер (`marksman`) — Одиночный урон, точность, пробивание брони и щитов
- ⚡ **Прицельный выстрел** (`aimedShot`): ✅ **Реализовано** (`server/arena/skills/aimedShot.ts`). Активный селф-бафф: увеличивает урон в `x1.25..x1.5` раза и **снимает/игнорирует увёртку цели** (`dodge`), гарантируя попадание.
- ⚡ **Бронебойный выстрел** (`piercingShot`): ✅ **Реализовано** (`server/arena/skills/piercingShot.ts`). Активный селф-бафф: **пробивает блок щитом** (`shieldBlock`), игнорирует 40–80% брони цели (`phys.defence`).
- ⚡ **Выстрел в колено** (`cripplingShot`): ✅ **Реализовано** (`server/arena/skills/cripplingShot.ts`). Активный селф-бафф: сбивает увёртку, снижает ловкость цели на 25–45% и блокирует уклонение на 2 раунда (*«Когда-то и меня вела дорога приключений...»*).
- ⚡ **Смертоносный выстрел** (`fatalShot`): 🗑️ **Исключено / Удалено** — убрано как неинтересный клон.
- 🛡️ **Метка охотника** (`huntersMark`): ✅ **Реализовано** (`server/arena/passiveSkills/huntersMark.ts`). *Пассивный талант*. При успешном выстреле с вероятностью 33–75% вешает на цель метку: входящий урон по ней увеличивается на +10..50% на 2 раунда.
- 🛡️ **Орлиный глаз** (`eagleEye`): ✅ **Реализовано** (`server/arena/passiveSkills/eagleEye.ts`). *Пассивный талант*. Замена мастерства дальнего оружия: с вероятностью 60–90% предотвращает промахи при стрельбе из лука.

#### 2. 🏹 Шквал (`barrage`) — Мульти-атаки, стихии (огонь + яд), рикошеты и боевой кураж
- ⚡ **Залп стрел** (`doubleShot`): ✅ **Реализовано** (`server/arena/skills/doubleShot.ts`). Активный селф-бафф: при физ. атаке дальнего боя лучник выпускает дополнительную стрелу во вторую живую цель (или ту же цель при дуэли) на 60–90% урона.
- ⚡ **Зажигательная стрела** (`fireArrow`): ✅ **Реализовано** (`server/arena/skills/fireArrow.ts`). Активный селф-бафф: заряжает следующую атаку дальнего боя огнем (доп. урон огнем) и поджигает цель (`burning`) периодическим уроном на 2 раунда.
- ⚡ **Отравленная стрела** (`poisonArrow`): ✅ **Реализовано** (`server/arena/skills/poisonArrow.ts`). Активный селф-бафф: смазывает стрелу ядом (доп. урон ядом) и отравляет цель (`poison`) периодическим уроном на 2 раунда.
- ⚡ **Град стрел** (`arrowRain`): 🗑️ **Исключено / Удалено** — убрано по запросу как лишнее умение.
- 🛡️ **Рикошет** (`ricochet`): ✅ **Реализовано** (`server/arena/passiveSkills/ricochet.ts`). *Пассивный талант*. При любом выстреле стрела с вероятностью 30–60% отскакивает во 2-го врага, нанося 50–70% урона.
- 🛡️ **Кураж** (`momentum`): ✅ **Реализовано** (`server/arena/passiveSkills/momentum.ts`). *Пассивный талант* (заменяет `secondAttack`). Каждое попадание восстанавливает +2..5 энергии и накапливает стаки куража (до 3 стаков, +5% урона за каждый).

#### 3. 🏃 Следопыт (`scout`) — Ловкость, уклонение, полевая медицина, тактический контроль
- ⚡ **Увёртка** (`dodge`): ✅ **Реализовано** (`server/arena/skills/dodge.ts`). Активный селф-заказ уклонения от атак колющим, режущим, рубящим, метательным и оглушающим оружием.
- ⚡ **Подножка** (`step`): ✅ **Реализовано** (`server/arena/skills/step.ts`). Активный тактический прием против врага (`OrderType.Enemy`). Срезает Защиту цели `wpr = wpr * level` (на 40–65%) и Атаку цели `wat = wat * (level + 0.2)` (на 20–45%) на текущий раунд.
- ⚡ **Спасение** (`help`): ✅ **Реализовано** (`server/arena/skills/help.ts`). Активный союзный навык (`OrderType.TeamExceptSelf`): прикрывает соратника, перенаправляя физический урон на себя со снижением на 15–35%.
- ⚡ **Полевая перевязка** (`fieldMedic` активная): 🗑️ **Исключено / Удалено** — активный дубль заменен мощной пассивкой.
- 🛡️ **Инстинкт выживания** (`survivalInstinct`): ✅ **Реализовано** (`server/arena/passiveSkills/survivalInstinct.ts`). *Пассивный талант*. Когда здоровье падает ниже 35%, весь получаемый урон снижается на 20–40%.
- 🛡️ **Полевой медик** (`fieldMedic`): ✅ **Реализовано** (`server/arena/passiveSkills/fieldMedic.ts`). *Пассивный талант* (заменяет `smokeScreen`). Увеличивает силу «Лечения руками» (`handsHeal`) на 30–60% и **полностью предотвращает срыв лечения** вражескими физическими атаками.
